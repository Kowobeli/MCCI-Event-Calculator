...
{
    public partial class sbWindow : Form
    {
        ...
        public void ShowData()
        {
            ...
            dtgSb.DataSource = dtMembers.AsDataView();
        ...
        private void sbWindow_Load(object sender, EventArgs e)
        {
            ShowData();
        }
        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            ...
            if (id != "")
            {
                string updateMembersSb = $"UPDATE {tableMembers} SET SbPoints = SbPoints + {totalPointsRounded} WHERE id = {id}";
                string updateMembersAllPoints = $"UPDATE {tableMembers} SET Points = Points + {totalPointsRounded} WHERE id = {id}";
                string updateTeamsSb = $@"UPDATE {tableTeams} SET SbPoints = COALESCE(SbPoints, 0) + {totalPointsRounded} WHERE Name = (SELECT TeamName FROM Members WHERE Id = {id} LIMIT 1)";
                ...
                int resultado = db.ExecuteCommands(updateMembersSb);
                int resultado2 = db.ExecuteCommands(updateMembersAllPoints);
                int resultado3 = db.ExecuteCommands(updateTeamsSb);
        ...
        private void dtgSb_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            lblId.Text = dtgSb.Rows[e.RowIndex].Cells[0].Value.ToString();
        }
        ...
        private void sbWindow_Click(object sender, EventArgs e)
        {
            txtPointsPerRound.ScrollBars = ScrollBars.None;
            txtPointsPerRound.Height = 20;
        }
