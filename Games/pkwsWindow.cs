...
namespace MCCI_Event_Calculator.Games
{
    public partial class pkwsWindow : Form
    {
        ...
        public void ShowData()
        {
            ...
            dtgPkws.DataSource = dtMembers.AsDataView();
        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            ...
            if (id != "")
            {
                string updateMembersPkws = $"UPDATE {tableMembers} SET PkwsPoints = PkwsPoints + {totalPointsRounded} WHERE id = {id}";
                string updateMembersAllPoints = $"UPDATE {tableMembers} SET Points = Points + {totalPointsRounded} WHERE id = {id}";
                string updateTeamsPkws = $@"UPDATE {tableTeams} SET PkwsPoints = COALESCE(BbPoints, 0) + {totalPointsRounded} WHERE Name = (SELECT TeamName FROM Members WHERE Id = {id} LIMIT 1)";
                ...
                int resultado = db.ExecuteCommands(updateMembersPkws);
                int resultado2 = db.ExecuteCommands(updateMembersAllPoints);
                int resultado3 = db.ExecuteCommands(updateTeamsPkws);
        ...
        private void pkwsWindow_Load(object sender, EventArgs e)
        {
            ShowData();
        }
        ...
        {
            lblId.Text = dtgPkws.Rows[e.RowIndex].Cells[0].Value.ToString();
        }
        ...
        private void pkwsWindow_Click(object sender, EventArgs e)
        {
            txtPointsPerRound.ScrollBars = ScrollBars.None;
            txtPointsPerRound.Height = 20;
        }
